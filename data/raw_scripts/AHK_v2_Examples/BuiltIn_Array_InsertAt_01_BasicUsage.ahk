...
Example6_ValidatedInsertion() {
    ...
    OutputDebug("`nUnique list: " FormatArray(uniqueList) "`n")

    if (!Contains(uniqueList, newValue)) {
        position := FindInsertPosition(uniqueList, newValue)
        uniqueList.InsertAt(position, newValue)
        OutputDebug("Inserted unique value " newValue ": " FormatArray(uniqueList) "`n")
    } else {
        OutputDebug("Value " newValue " already exists`n")
    }
...
Example7_PracticalUseCases() {
    ...
    ; Use Case 3: Insert page in document
    pages := ["Cover", "Table of Contents", "Conclusion"]
    OutputDebug("`nDocument pages: " FormatArray(pages) "`n")

    pages.InsertAt(3, "Chapter 1", "Chapter 2", "Chapter 3")
    OutputDebug("After inserting chapters: " FormatArray(pages) "`n")
 ...
 * @param {Any} value - Value to insert
 * @returns {Integer} Position to insert
 */
