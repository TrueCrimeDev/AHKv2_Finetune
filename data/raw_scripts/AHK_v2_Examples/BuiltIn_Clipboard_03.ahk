...
class ClipboardAnalyzer {
    ...
    static Analyze() {
        ...
        if (FilePathDetector.IsFilePath())
            analysis["formats"].Push("File Path")

        if (NetworkDetector.IsURL())
            analysis["formats"].Push("URL")

        if (NetworkDetector.IsEmail())
            analysis["formats"].Push("Email")

        if (StructuredDataDetector.IsJSON())
            analysis["formats"].Push("JSON")

        if (StructuredDataDetector.IsXML())
            analysis["formats"].Push("XML")

        if (StructuredDataDetector.IsHTML())
            analysis["formats"].Push("HTML")

        if (StructuredDataDetector.IsCSV())
            analysis["formats"].Push("CSV")

        if (CodeDetector.IsCode()) {
            lang := CodeDetector.DetectLanguage()
            analysis["formats"].Push("Code (" . lang . ")")
        }

        if (NumberDateDetector.IsNumber())
            analysis["formats"].Push("Number")

        if (NumberDateDetector.IsDate())
            analysis["formats"].Push("Date")
    ...
    static ShowAnalysis() {
        ...
        if (analysis["isEmpty"]) {
            MsgBox("Clipboard is empty!", "Clipboard Analyzer", "Icon Info")
            return
        }

        ; Create GUI
        gui := Gui("+Resize", "Clipboard Content Analyzer")
        gui.SetFont("s10")

        ; Statistics
        gui.Add("GroupBox", "w500 h120", "Statistics")
        stats := analysis["stats"]
        gui.Add("Text", "xp+10 yp+25", "Characters:")
        gui.Add("Text", "x+10", stats["chars"])
        gui.Add("Text", "x20 y+5", "Words:")
        gui.Add("Text", "x+50", stats["words"])
        gui.Add("Text", "x20 y+5", "Lines:")
        gui.Add("Text", "x+55", stats["lines"])

        ; Detected Formats
        gui.Add("GroupBox", "x10 y+20 w500 h150", "Detected Formats")
        formatList := gui.Add("ListBox", "xp+10 yp+25 w480 h110")
        ...
        ; Preview
        gui.Add("GroupBox", "x10 y+20 w500 h200", "Preview")
        preview := SubStr(A_Clipboard, 1, 500)
        if (StrLen(A_Clipboard) > 500)
            preview .= "..."
        gui.Add("Edit", "xp+10 yp+25 w480 h160 ReadOnly", preview)

        ; Close button
        btnClose := gui.Add("Button", "x220 y+10 w100", "Close")
...
F7::ClipboardAnalyzer.ShowAnalysis()
...
F12:: {
    ...
    (
    ╔════════════════════════════════════════════════════════════════╗
    ║           CLIPBOARD FORMAT DETECTION - HOTKEYS                 ║
    ╠════════════════════════════════════════════════════════════════╣
    ║                                                                ║
    ║ F1                  Show basic clipboard statistics            ║
    ║ F2                  Analyze file paths                         ║
    ║ F3                  Analyze URLs/Emails                        ║
    ║ F4                  Detect structured data formats             ║
    ║ F5                  Detect code language                       ║
    ║ F6                  Analyze numbers and dates                  ║
    ║ F7                  Comprehensive clipboard analysis           ║
    ║                                                                ║
    ║ F12                 Show this help                             ║
    ║                                                                ║
    ╚════════════════════════════════════════════════════════════════╝
    ...
    MsgBox(helpText, "Format Detection Help", "Icon Info")
}
